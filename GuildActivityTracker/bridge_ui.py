...
class BridgeUI:
    def __init__(
        ...
        self.autostart_available = autostart_available
    ...
    def _build_ui(self):
        ...
        settings.pack(fill="x", padx=10, pady=(4, 0))
        if self.on_toggle_autostart and self.autostart_available:
            ...
            tk.Checkbutton(
                ...
                bg=t.background,
                fg=t.foreground,
                activebackground=t.background,
                activeforeground=t.accent,
                selectcolor=t.button_bg,
                anchor="w",
                pady=4,
            ).pack(fill="x")
        elif not self.autostart_available:
            tk.Label(
                settings,
                text="Inicio automático disponible solo en Windows.",
                bg=t.background,
                fg=t.muted,
                anchor="w",
                padx=2,
    ...
    def _toggle_autostart(self):
        if not self.on_toggle_autostart or self.autostart_var is None:
            return
        desired = bool(self.autostart_var.get())
        try:
            success = self.on_toggle_autostart(desired)
        except Exception:
            success = False
        if not success:
